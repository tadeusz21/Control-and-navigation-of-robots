...
int main(int argc, char* argv[])
{
  ...
  Eigen::Isometry3d T_z_90= Eigen::Translation3d(0.0, 0.0, 0.0) *
  Eigen::Quaterniond(0.0, 0.0, 0.71, 0.71).normalized();
  Eigen::Isometry3d T_x_90= Eigen::Translation3d(0.0, 0.0, 0.0) *
  Eigen::Quaterniond(0.71, 0.0, 0.0, 0.71).normalized();
    Eigen::Isometry3d T_x_30= Eigen::Translation3d(0.0, 0.0, 0.0) *
  Eigen::Quaterniond(0.26, 0.0, 0.0, 0.97).normalized();

    // Tablica na wyniki
std::vector<Eigen::Isometry3d> cube_pose_array;

// Generowanie wszystkich kombinacji transformacji
for (int i = 0; i < 6; ++i) {
    for (int j = 0; j < 4; ++j) {
        for (int k = 0; k < 4; ++k) {
            Eigen::Isometry3d transformed_pose = cube_pose;

            if (i == 1) transformed_pose = transformed_pose * T_x_90;
            if (i == 2) transformed_pose = transformed_pose * T_x_90*T_x_90;
            if (i == 3) transformed_pose = transformed_pose * T_x_30*T_x_90*T_x_90;
            if (i == 4) transformed_pose = transformed_pose * T_z_90*T_x_90;
            if (i == 5) transformed_pose = transformed_pose * T_x_30*T_x_90*T_x_90*T_x_90;
            

            if (j == 1) transformed_pose = transformed_pose * T_z_90;
            if (j == 2) transformed_pose = transformed_pose * T_z_90*T_z_90;
            if (j == 3) transformed_pose = transformed_pose * T_z_90*T_z_90*T_z_90;

            

            if (k == 1) transformed_pose = transformed_pose * T_x_90;
            if (k == 2) transformed_pose = transformed_pose * T_x_30;
            // if (k == 2) transformed_pose *= T_x_30;

            // Dodajemy wynik do tablicy
            cube_pose_array.push_back(transformed_pose* T_CE * T_spin);
        }
    }
}


for (size_t i = 0; i < cube_pose_array.size(); ++i) {
    double axis_size = 0.5;
    double axis_marker_scale = 0.01;
    std::string marker_name = "end_effector_" + std::to_string(i);  // Unikalna nazwa markera

    moveit_visual_tools.publishAxis(cube_pose_array[i], axis_size, axis_marker_scale, marker_name);

    // moveit_visual_tools.publishAxis(pre_grip_pose, 0.1, 0.05, "pre_grip_" + std::to_string(i));
